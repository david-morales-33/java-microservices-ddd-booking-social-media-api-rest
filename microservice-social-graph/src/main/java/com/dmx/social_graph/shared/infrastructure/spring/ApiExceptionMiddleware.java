...
public final class ApiExceptionMiddleware implements Filter {
    ...
    private void handleCustomError(
        ...
        HashMap<Class<? extends DomainException>, HttpStatus> errorMapping = possibleController
            .errorMapping();
        Throwable error = (
            exception.getCause() instanceof CommandHandlerExecutionException ||
            exception.getCause() instanceof QueryHandlerExecutionException
        )
    ...
    private String errorCodeFor(Throwable error) {
        if (error instanceof DomainException) {
            return ((DomainException) error).errorCode();
        }

        return Utils.toSnake(error.getClass().toString());
    }

    private int statusFor(HashMap<Class<? extends DomainException>, HttpStatus> errorMapping, Throwable error) {
